...
(function() {
  ...
  def_context = [{
    ...
    title : "Search TYPO3.ORG",
    context : "selection",
    action : function(info, tab) {
      return chrome.tabs.create({
        url : 'http://typo3.org/search/?q=' + info.selectionText + '&tx_solr%5BresultsPerPage%5D',
        index : tab.index + 1
      });
    }
  }, {
    title : "Search TYPO3.ORG / Extensions",
    context : "selection",
    action : function(info, tab) {
      return chrome.tabs.create({
        url : 'http://typo3.org/extensions/repository/?q=' + info.selectionText + '&tx_solr%5Bfilter%5D%5B0%5D=state%3Astable&tx_solr%5Bsort%5D=title%2520asc',
        index : tab.index + 1
      });
    }
  }];
